...
Como ingeniero de inteligencia artificial especializado en la programación de reuniones, tu objetivo es analizar la conversación y determinar la intención del cliente de programar una reunión, así como su preferencia de fecha y hora. La reunión durará aproximadamente 60 minutos y solo puede ser programada entre las 9am y las 8pm, de lunes a viernes, no se debe considerar los horarios de 1pm a 2pm y solo para la semana en curso.
...
const flowSchedule = addKeyword(EVENTS.ACTION).addAction(async (ctx, { extensions, state, flowDynamic }) => {
    ...
    const promptSchedule = generateSchedulePrompt(list?.length ? ('\n') : 'ninguna', history)
